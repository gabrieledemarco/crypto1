...
## Pareto Terminal — Project Context

**Stack**: Next.js 14 + FastAPI + DuckDB on Railway  
**Repo**: `gabrieledemarco/crypto1` — branch `fix/web-build-errors`  
**Web**: `https://humorous-insight-production-340f.up.railway.app`  
**API**: `https://illustrious-inspiration-production.up.railway.app`

### Architecture
```
Browser → Next.js /app/api/[...path]/route.ts (runtime proxy, reads API_URL env) → FastAPI
FastAPI → DuckDB (/data/pareto.db on Railway volume)
FastAPI → yfinance (blocked from this dev env — works on Railway)
```

### Key files
| File | Role |
|------|------|
| `api/main.py` | FastAPI app + router mounts |
| `api/routers/runs.py` | Backtest + equity/trades/wfo/sweep/mc + SSE |
| `api/routers/assets.py` | Asset fetch (yfinance) + bars + stats |
| `api/routers/vibe.py` | Claude streaming proxy |
| `web/app/api/[...path]/route.ts` | Runtime proxy (forwards query string!) |
| `web/hooks/useRun.ts` | React Query hooks + `normalizeApiTrade()` |
| `web/store/index.ts` | Zustand state — `activeRunId`, `runs`, `screen` |
| `web/lib/fixtures.ts` | Fixture data + `Trade`/`EquityPoint`/`Run` types |

### Screens (10 total)
Dashboard · Equity · Trades · Setup · Sweep · MonteCarlo · Underwater · Assets · Library · Vibe

### Ruflo memory keys (this project)
```bash
npx ruflo@latest memory search -q "project/"
# project/context — full project metadata
# project/api_routes — all FastAPI endpoints
# project/known_bugs_open — minor issues pending fix
```

### Environment constraints
- Railway API/Web: blocked from this env (`x-deny-reason: host_not_allowed`)
- yfinance: blocked from this env (same issue)
- Local FastAPI on `:8099` available for unit testing
- For browser testing: Playwright MCP configured (`npx @playwright/mcp@latest`)

### Agent routing for this project
| Task | Ruflo pattern | Agents |
|------|--------------|--------|
| Fix a bug | Sequential: researcher → coder → tester | `coder`, `tester` |
| Audit all screens | Fan-out: 5 parallel screen reviewers | `researcher` × N |
| Test endpoints | Fan-out: API tester + browser tester | `tester`, `browser` |
| New feature | Hierarchical: architect → coder → reviewer | `system-architect`, `coder`, `reviewer` |
